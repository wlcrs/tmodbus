"""Example of an asynchronous RTU over TCP Modbus client using tmodbus."""

import asyncio

from tmodbus import create_async_rtu_over_tcp_client
from tmodbus.exceptions import InvalidResponseError, ModbusConnectionError, ModbusResponseError


async def example_rtu_over_tcp_client() -> None:
    """Asynchronous RTU over TCP Modbus client example."""
    ...
    client = create_async_rtu_over_tcp_client(host, port, unit_id=unit_id)
    ...
    async with create_async_rtu_over_tcp_client(host, port, unit_id=unit_id) as client2:
        print("Status of coils 0-7: ", await client2.read_coils(start_address=0, quantity=8))


if __name__ == "__main__":
    asyncio.run(example_rtu_over_tcp_client())
