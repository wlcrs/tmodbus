"""Example of an asynchronous ASCII Modbus client using tmodbus."""

import asyncio

from tmodbus import create_async_ascii_client
from tmodbus.exceptions import InvalidResponseError, ModbusConnectionError, ModbusResponseError


async def example_ascii_client() -> None:
    ...
    # The create_async_ascii_client function returns an instance of AsyncModbusClient
    client = create_async_ascii_client(port, baudrate=baudrate, unit_id=unit_id)
    ...
    async with create_async_ascii_client(port, baudrate=baudrate, unit_id=unit_id) as client2:
        print("Status of coils 0-7: ", await client2.read_coils(start_address=0, quantity=8))


if __name__ == "__main__":
    asyncio.run(example_ascii_client())
