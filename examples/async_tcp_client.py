...
async def example_tcp_client() -> None:
    ...
    try:
        ...
        response = await client_for_unit_id_2.read_holding_registers(start_address=100, quantity=2)
        print("Contents of holding registers 100 and 101 for unit ID 2: ", response)

        response = await client.for_unit_id(6).read_float(start_address=1)
        print("Float value at address 1 for unit ID 6: ", response)
